 ...
 */
export const getUserHistory = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
 ...
 */
export const clearUserHistory = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    // Extract user ID from authorization header
    const authHeader = event.headers?.Authorization || event.headers?.authorization;
    const userId = await getUserIdFromToken(authHeader);
